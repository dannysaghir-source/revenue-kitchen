...
export default function Contact() {
  ...
  useEffect(() => {
    ...
    if (!w.Cal) {
      w.Cal = function () { (w.Cal.q = w.Cal.q || []).push(arguments); };
      w.Cal.q = [];
    }
    w.Cal.ns = w.Cal.ns || {};
    if (!w.Cal.ns["30min"]) {
      w.Cal.ns["30min"] = function () { (w.Cal.ns["30min"].q = w.Cal.ns["30min"].q || []).push(arguments); };
      w.Cal.ns["30min"].q = [];
    }
  ...
  return (
    <section id="contact" style={base.section}>
      ...
      <div id="my-cal-inline-30min" style={{ display: status === "contact" ? "block" : "none", width: "100%", maxWidth: "900px", minHeight: "600px", overflow: "scroll" }} />
    </section>
  );
}"use client";
import { useEffect, useState } from "react";
